...
```
このとき，`Sepal.Length` という名前のオブジェクトを探しに行き，それがないためエラーを返しています。
...
リストはさまざまなオブジェクトを入れることのできる，引き出しだと説明されることがあります。
さまざまなオブジェクトをリストに入れることができます。
...
リストは，Excelファイルにたとえることもできます。
Excelの1つのファイルの中に複数のシートがありますよね。
Rのリストも同じように，複数のオブジェクト（Excelのシートのようなもの）が入っていると考えると理解しやすいです。
リストの中には，データフレームだけでなく，ベクトルや行列，さらには他のリストも入れることができます。
...
J.D. Long and Paul Teetor（2020）[『Rクックブック 第2版』](https://www.oreilly.co.jp/books/9784873118857/){target="_blank"}オライリー・ジャパンは，Rの基本的な使い方が分かりやすく説明されています。
情報量が多いため，じっくり読む価値があります。
...
4. 次のオブジェクト `x` から `NA` を除いて平均を求めなさい。

``` r
x <- c(1, 2, NA, 4, NA, 6)
```

5. 次のオブジェクト `x` を，水準の順序を「小 < 中 < 大」に設定したファクターに変換しなさい。

``` r
x <- c("中", "小", "大", "中", "大", "小")
```

6. 次のリスト `x` と `y` のそれぞれから松山市の人口の値を取り出しなさい。
...
10. 気象庁の次のWebサイトからCSVファイルをダウンロードして，オブジェクト `temp` に読み込んだ後，世界全体の年平均気温偏差の最大値と最小値を求めなさい。
