...
Hadley Wickham, Mine Çetinkaya-Rundel, and Garrett Grolemund（2023）[『Rではじめるデータサイエンス 第2版』](https://www.oreilly.co.jp/books/9784873118857/){target="_blank"}オライリー・ジャパンは，tidyverse の観点からRの基本的な使い方を説明していますので，参考にしてください。
...
上に示したコードのいずれにも共通するのが，新たなオブジェクトを作成している点です。
コードを書くようになると，作業途中に必要なオブジェクトの名前を考えるのが面倒になってきます。
こうしたオブジェクトが少ない場合は特に問題となりませんが，多くなってくると意味のあるオブジェクト名がなかなか思い浮かびません。
どうすればよいか正解があるわけではないし，いちいち考えるのが面倒なので，`tmp` とか `df2` や `df3` といったオブジェクト名にしがちですが，どの場合も後々に見て意味が分かりにくいです。
...
処理結果を新しいオブジェクトに代入する場合は，base R の作法に従います。
...
tidyverse についてより本格的に学びたい場合，次の書籍を参考にしてください。

- Hadley Wickham，Mine Çetinkaya-Rundel，Garrett Grolemund（2023）[『Rではじめるデータサイエンス 第2版』](https://www.oreilly.co.jp/books/9784814400775/){target="_blank"}オライリー・ジャパン

本書の英語版は以下のサイトから無料で読めます。

- [R for Data Science (2e)](https://r4ds.hadley.nz){target="_blank"}
- [R for Data Science](https://r4ds.had.co.nz){target="_blank"} # 初版


## 練習問題

1. 次のデータフレーム `x` があるとする。`z` を使って，2000年以降の `value` の値の平均を求めなさい。ただし，ここでは tidyverse を用いなさい。

``` r
x <- tibble(
  year = 1990:2021,
  value = seq(5000, 5030, 2)
)
```
