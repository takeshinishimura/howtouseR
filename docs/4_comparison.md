...
```
ここで，ベクトルの各要素1つずつに対して比較していることを理解してください。
...
## [16] 2005 2006 2007 2008 2009 2010
```
このベクトルの1つ目の要素と `2000` を比較した結果が `year > 2000` の返り値のベクトルの1つ目の要素 `FALSE` に対応しています。
...
```
##  [1]  TRUE FALSE FALSE FALSE FALSE  TRUE  TRUE    NA FALSE  TRUE
```
...
1. 次のデータフレーム `z` があるとする。`z` を使って，2000年以降の `value` の値の平均を求めなさい。[基礎](basics.html)の練習問題と同じです。ただし，ここでは比較演算子を用いなさい。
