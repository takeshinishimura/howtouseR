...
# 古いファイル形式
d <- read.csv("../data/Meter_data20230402.csv")
dts <- xts::as.xts(zoo::read.zoo(d))
head(dts)
dim(dts)

# 最近のファイル形式だと次のようにする必要がある。
# ただし，データが綺麗に取れていないのか，毎月上旬の値しかないようである。
# d <- read.csv("../data/Meter_data20240127.csv", check.names = FALSE)
# d$Timestamp <- lubridate::mdy_hms(d$Timestamp, tz = Sys.timezone())# 'Asia/Tokyo'
# duplicates <- d[duplicated(d$Timestamp), ]
# d_unique <- d[!duplicated(d$Timestamp), ]
# dts <- xts::as.xts(zoo::read.zoo(d_unique, index.column = "Timestamp"))
# head(dts)
# dim(dts)
```
...
plot(dts[, "Temperature_Celsius"], main = "Temperature Celsius (°C)")
...
plot(dts["2022", "Temperature_Celsius"], main = "Temperature Celsius (°C)")
```
...
plot(dts["2022-12/2023-01", "Temperature_Celsius"], main = "Temperature Celsius (°C)")
```
:::
...
layout(matrix(1:2, ncol=1))
plot(cbind(apply.monthly(dts[, "Temperature_Celsius"], min), apply.monthly(dts[, "Temperature_Celsius"], max)), main = "Temperature Celsius (°C)")
plot(apply.monthly(dts[, "Temperature_Celsius"], mean), main = "Temperature Celsius (°C)")
```
