...
文字コードが UTF-8 でない場合に UTF-8 に変換するようにすればよいでしょう。
```{r}
#| eval: false
iconv(textdata, from = "CP932", to = "UTF-8")
```

また，表記揺れの修正は日本語のテキストマイニングにおいて重要です。
例えば，次のようにします。
```{r}
#| eval: false
...
```
濁点と半濁点以外の正規化は，次のコマンドでできるようです（詳細な確認はしていません）。
```{r}
#| eval: false
library(audubon)
...
Windows の場合，インストール時に辞書の文字コードを選択する画面が表示されます。
迷ったら，URF-8 でよいでしょう。
macOS の場合は Source をダウンロードして，UNIX と同じ方法でインストールしてください。

以下では，R から MeCab を使うための複数の方法を紹介します。
...
process_book <- function(url, bookname) {
  ...
  text <- rvest::read_html(temp_file, encoding = "Shift_JIS") |>
  ...
  text <- trimws(text)
  text_norubi <- gsub("（[^）]*）", "", text)
  paragraphs <- str_split(text_norubi, "\n")

  df <- tibble(text = unlist(paragraphs), book = bookname)

  return(df)
}

# 坊ちゃん
botchan_df <- process_book(
  "https://www.aozora.gr.jp/cards/000148/files/752_14964.html",
  "坊ちゃん"
)

# 羅生門
rashomon_df <- process_book(
...
自分のパソコンで以下のコマンドを実行する場合，ファイルパスに注意し，自分の環境に合ったパスに変更してください。
```{r}
#| message: false
library(readr)

txtdir <- "../data/sample_text"
dir.create(txtdir, showWarnings = FALSE)

readr::write_lines(botchan_df$text, file.path(txtdir, "坊ちゃん.txt"))
readr::write_lines(rashomon_df$text, file.path(txtdir, "羅生門.txt"))
```
...
```{r}
#| message: false
filename <- file.path(txtdir, "坊ちゃん.txt")

text_freq <- RMeCabFreq(filename)
head(text_freq)
...
```{r}
tdm <- docMatrix(txtdir, pos = c("名詞", "形容詞", "副詞", "動詞"), weight = "")
head(tdm)
...
wordcloud(
  word = row.names(word_freqs_20),
  freq = word_freqs_20$freq,
  random.order = FALSE,
  rot.per = 0,
...
```{r}
textdata_two <- textdata |>
  summarise(text = paste0(text, collapse = " "), .by = book)

textdata_two

library(tm)

word_list <- RMeCabC(textdata_two$text[1])
corp <- tm::Corpus(VectorSource(word_list))
(dtm <- tm::DocumentTermMatrix(corp))

word_freqs <- colSums(as.matrix(dtm))

data.frame(term = names(word_freqs), freq = word_freqs) |>
  with(
    wordcloud(
      words = term,
      freq = freq,
...
macOS の場合，`added.csv` を UTF-8 で作成し，"~/Documents" に保存します。
...
Windows の場合，`added.csv` を UTF-8 で作成し，"C:\Program Files (x86)\MeCab\dic\ipadic" に保存します。
...
textdata_3 <- tibble(filenames = list.files(txtdir, full.names = TRUE)) |>
  mutate(
    .keep = "none", 
...
```{r}
#| eval: false
text_pos <- RcppMeCab::pos(textdata_3$text, format = "data.frame")
text_pos[3:23, ]
```
...
macOS で辞書の場所が見つからないというエラーが出る場合，`Sys.setenv(MECABRC = "/opt/homebrew/etc/mecabrc")` を実行してみるとよいかもしれません。
もしこれで上手くいくようなら，`usethis::edit_r_environ()` を実行して，開いたファイルに `MECABRC=/opt/homebrew/etc/mecabrc` という1行を追加します。
...
ここで，macOS だと次のエラーメッセージとともにエラーが返ってくることがある。
```
 *** caught segfault ***
...
```{r}
textdata_ja <- textdata |>
  filter(book == "坊ちゃん") |>
  mutate(
    doc_id = seq_along(text) |> as.character(),
    text = audubon::strj_normalize(text)
  )

text_token <- gibasa::tokenize(textdata_ja, text, doc_id)

reactable::reactable(text_token, compact = TRUE)

text_token |>
  gibasa::prettify(col_select = c("POS1", "Original")) |>
  filter(POS1 %in% c("名詞", "形容詞", "副詞", "動詞"))
```

トークンをまとめて新しいトークンにすることができる。
```{r}
reactable::reactable(
  gibasa::prettify(text_token, col_select = c("POS1", "POS2", "POS3")),
  compact = TRUE
)
reactable::reactable(
  gibasa::prettify(text_token, col_select = c("POS1", "POS2", "POS3")),
...
text_token |>
  ...
  gibasa::collapse_tokens(
    (POS1 %in% c("名詞", "形容詞", "副詞", "動詞") & !str_detect(token, "^[あ-ン]+$")) |
...
dat_count <- text_token |>
  gibasa::prettify(col_select = c("POS1", "POS2", "POS3", "Original")) |>
  gibasa::collapse_tokens(
    (POS1 %in% c("名詞", "形容詞", "副詞", "動詞") & !str_detect(token, "^[ァ-ヴ\\s]+$")) |
    (POS1 %in% c("名詞", "形容詞") & POS2 %in% c("自立", "接尾", "数接続"))
  ) |>
  mutate(
...
```{R}
path <- "../data/sample_text_en"
filenames <- list.files(path, full.names = TRUE)
text_en <- list()
for (i in filenames) {
  text_en[[sub(paste0(path, "/"), "", i)]] <- readLines(i)
}

textdata_en <- paste(unlist(text_en), collapse = "")
```
...
tidytext::stop_words
```
ストップワードを辞書ごとに `filter()` することも可能です。
```{r}
...
```{r}
text_df <- tibble(text = unlist(text_en))

text_token <- text_df |>
  tidytext::unnest_tokens(output = token, input = text)

mystopwords <- tibble(
  word = c("eq", "co", "rc", "ac", "ak", "bn", "fig"),
  lexicon = "me"
)

new_text_token <- text_token |>
  anti_join(
    tidytext::stop_words |> bind_rows(mystopwords),
    by = c("token" = "word")
  )
```

`unnest_tokens()` では句読点，記号は削除され，小文字に変換されます。
```{r}
